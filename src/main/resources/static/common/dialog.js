...
function openDatasourceDialog(callBack){
	var url=ajaxUrl+"/sys/core/sysDatasource/dialog.do";
	mini.open({
		url : url,
		title : "选择数据源",
		width : 650,
...
function openCustomQueryDialog(callBack){
	var url=ajaxUrl+"/sys/db/sysDbSql/dialog.do";
	mini.open({
		url : url,
		title : "选择自定义SQL",
		width : 800,
...
function openBpmSolutionDialog(single,callBack){
	...
	_OpenWindow({
		url: ajaxUrl + "/bpm/core/bpmSolution/dialog.do?single="+strSingle,
...
function openBoDefDialog(supportType,callBack,multi){
	var url=ajaxUrl+'/sys/bo/sysBoDef/dialog.do';
...
function openBpmFormViewDialog(conf){
	...
	var url=ajaxUrl+'/bpm/form/bpmFormView/dialog.do?single='+ single;
	//如果选择bo定义。
	if(bodefId){
...
function openBpmMobileFormDialog(conf){
	var single=conf.single?"true":"false";
	var bodefId=conf.bodefId;
	var url=ajaxUrl+'/bpm/form/bpmMobileForm/dialog.do?single='+ single;
	//如果选择bo定义。
	if(bodefId){
		url+='&boDefIds=' +bodefId;
	}
...
function _IconSelectDlg(callback){
	_OpenWindow({
		url:ajaxUrl+'/sys/core/file/iconSelectDialog.do',
		height:340,
		width:506,
		iconCls:'icon-user',
		title:'图标选择',
...
function openFlowChartDialog(conf){
	var url=ajaxUrl+"/bpm/core/bpmInst/flowChart.do";
	if(conf.solId){
...
function openProfileDialog(conf){
	...
	var url=ajaxUrl + "/sys/core/public/profileDialog.do";
	if(hideRadio=="YES"){
		url+="?hideRadio=YES";
	}
	_OpenWindow({
